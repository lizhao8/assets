...
//		gl.glEnd(); // Done Drawing The Quad
		...
		/*for (int i = 0; i < mesh.m_Indices.size(); i += 3) {
			...
			gl.glVertex3f(-mesh.m_Vertices.get(long2*3+0).value,
					mesh.m_Vertices.get(long2*3+1).value,
					mesh.m_Vertices.get(long2*3+2).value);
...
*/
		...
		float b = 0;
		gl.glBegin(GL2.GL_TRIANGLES);
		for (int i = 0; i < mesh.elementList.size(); i++) {
			Element element = mesh.elementList.get(i);
			if (index % mesh.elementList.size() == i) {
				if (Math.sqrt(index) >= mesh.elementList.size()) {
					index = 0;
				}
				gl.glColor3f(1, 0, 0);
			} else {
